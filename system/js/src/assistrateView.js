...
function filterdata(){
	...
	var caption = "As of "+ stoday;
        ...
        "columns": [
			...
			{ "data": "status"},
			{ "className":'editbutton',"defaultContent": '<button type="button" data-toggle="tooltip" title="Assistance Rate Status" class="btn btn-link" ><span><i class="fa fa-pencil" aria-hidden="true"></i></span></button>'}	
			],
        "order": [[1, 'asc']],
...
function format(row){
	...
	var xcontent = '<div id="'+divid+'" class="row">';
    xcontent += '<div class="col-12 text-end"><button type="button" name="addbutton" onclick="showNewAssistance('+code+', '+idx+')"  class="btn btn-primary addbutton" data-toggle="tooltip" title="Add Rate Schedule"><span class="fa fa-folder-plus"></span></button></div>';						
    xcontent += '<table class="table table-sm" id="rateCode'+id+'" style="width:100%">';
...
function getchild(data){
	...
	var table = $(tableid).DataTable( {"destroy":true,"searching":false,
        ...
        "columns": [
			...
			{ "data": "assistStatus"},
			{ "className":'editbutton',"defaultContent": '<button type="button" data-toggle="tooltip" title="Assistance Sched Status" class="btn btn-link" ><span><i class="fa fa-pencil" aria-hidden="true"></i></span></button>'},	
		],
        "order": [[1, 'asc']]
    } );
	$(tableid+' tbody').off();
