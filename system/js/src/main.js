   ...
   if (data['userid']==-1){
		...
		// **************************************************** 10/26/2022 [added - end]		
			...
			try {
				...
				setTimeout(function(){ startload(), 9000});
			}
			});
...
function showmenu(userid, token){
	$.get("controllers/usersController.php", {trans:"priviledges",userid: userid,"tk":token}, function(data) {showmenu2(data);},'json');
}
		...
		if (key != "icon"){
			...
			var details = content[key];
				...
				//}
					...
					detail += '<div class="col col-sm-6" style="margin-bottom: 5px;"><div class="card"><div class="card-body text-center"><a href="'+xurl+ '"><i class="'+icon+'"></i><br><strong>'+key+'</strong></a></div></div></div>';
