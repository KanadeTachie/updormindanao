...
function startload() {
	...
	var table = $('#ds').DataTable( {
        ...
        "columns": [
			...
			{ "data": "fullname"},
			{ "data": "officename"},
			{"className":'rolebutton',"defaultContent": '<button type="button" data-toggle="tooltip" title="Roles" class="btn btn-link" ><span class="fas fa-user-tag" aria-hidden="true"></span></button>'},	

			{"className":'prevbutton',"defaultContent": '<button type="button" data-toggle="tooltip" title="priviledges" class="btn btn-link" ><span class="fa fa-lock" aria-hidden="true"></span></button>'}	
...
var tk=qs['tk'];
	...
	sheader ="<div class='row'><div class='col-md-2'><img src='"+src+"' style='max-width:5em;'></div><div class='col-md-10'>";
	key =data['fullname'];
	sheader += "Name: <strong>"+key+"</strong>";
	sheader +=" Email: <strong>"+data['emailaddress']+"</strong><br>";
	
	sheader +=" Active: <strong>"+data['active']+"</strong></div></div>";
...
for(var key in menu) {
			...
			if (detail['url']){
				...
				content +="<div class='col-12 text-center'><input type='checkbox' name='menu[]' value='"+key+"' "+c+" >"+key+"</div><div class='row'><br><br></div>";
			} else {
				content +="<div class='col-12 text-center'>"+key+"</div>";
				content += showlist(key,access);
				content +="<div class='row'><br><br></div>";
			}
}
content+="<div class='col-12'><button class='btn btn-primary' type='button' onclick='saveaccess()'>Save</button></div>";
content+="</div></form>";
...
var content='<form id="rolefrm" style="width:100%"><div class="row"><input type="hidden"  name="userid" value="'+id+'"><input type="hidden" name="trans" value="roles"><input type="hidden" name="tk" value="'+qs['tk']+'">';
for (a = 0;a < allroles.length; a++){ 
				var key = allroles[a];
				var c = checkrole(key['idroles'],roles);
				let idrole = key['idroles'];
				let details = key['name'];
				content +="<div class='col-12 text-center'><input type='checkbox' name='roles[]' value='"+idrole+"' "+c+" >"+details+"</div>";
			
}
content+="<div class='col-12'><button class='btn btn-primary' type='button' onclick='saveroles()'>Save</button></div>";
content+="</div></form>";
...
var stitle = menu[key];
 for(var details in stitle){
	...
	sdetails +="<div class='col-4'><input type='checkbox' name='menu[]' value='"+details+"' "+c+" >"+details+"</div>";
 }
return sdetails;
}
function saveaccess(){
